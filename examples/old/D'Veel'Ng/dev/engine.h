...
void process_tile (unsigned char x0, unsigned char y0, unsigned char x1, unsigned char y1) {
#ifdef PLAYER_PUSH_BOXES
#ifdef FIRE_TO_PUSH
	gpit = (joyfunc) (&keys);
	if ((gpit & sp_FIRE) == 0) {
#endif		
		if (qtile (x0, y0) == 14 && attr (x1, y1) == 0 && x0 > 0 && x0 < 14 && y0 > 0 && y0 < 9) {
			// Mover
...
unsigned char move (void) {
	...
	*/
...
#ifdef PLAYER_CAN_FIRE
	if (((gpit & sp_UP) == 0) && ((player.vy == 0 && player.saltando == 0 && (attr (gpxx, gpyy + 1) > 3 || ((gpx & 15) && attr (gpxx + 1, gpyy + 1) > 3))) || player.gotten)) {
		player.saltando = 1;
		player.cont_salto = 0;
		peta_el_beeper (3);
	}
	...
	if ((gpit & sp_UP))
		player.saltando = 0;
#else
	if (((gpit & sp_FIRE) == 0) && ((player.vy == 0 && player.saltando == 0 && (attr (gpxx, gpyy + 1) > 3 || ((gpx & 15) && attr (gpxx + 1, gpyy + 1) > 3))) || player.gotten)) {
		player.saltando = 1;
		player.cont_salto = 0;
		peta_el_beeper (3);
	}
