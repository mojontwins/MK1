	...
	while (1) {
		...
		while (playing) {
...
#endif
				sp_MoveSprAbs (sp_moviles [gpit], spritesClip, en_an_next_frame [gpit] - en_an_current_frame [gpit], VIEWPORT_Y + (y >> 3), VIEWPORT_X + (x >> 3),x & 7, y & 7);
			...
			if ( !(pestado & EST_PARP) || !(half_life) )
				sp_MoveSprAbs (sp_player, spritesClip, pnext_frame - pcurrent_frame, VIEWPORT_Y + (y >> 3), VIEWPORT_X + (x >> 3), x & 7, y & 7);
			else
			...
			if (px == 14336 && pvx > 0 && x_pant < (level_data->map_w - 1)) {	
#else			
			if (px == 14336 && pvx > 0 && x_pant < (MAP_W - 1)) {	
#endif
				n_pant ++;
				x_pant ++;
				draw_scr ();
...
#if defined (MODE_128K) && defined (COMPRESSED_LEVELS)
			if (py == 9216 && pvy > 0 && y_pant < (level_data->map_h - 1)) {
				n_pant += level_data->map_w;
#else			
			if (py == 9216 && pvy > 0 && y_pant < (MAP_H - 1)) {
				n_pant += MAP_W;
#endif
				y_pant ++;
				draw_scr ();
...
#else
			if (px == 0 && ((gpit & sp_LEFT) == 0)) {
				n_pant --;
				draw_scr ();	
				px = 14336;
			}
			if (px == 14336 && ((gpit & sp_RIGHT) == 0)) {		// 14336 = 224 * 64
				n_pant ++;
				draw_scr ();
				px = 0;
			}
#endif
#if defined (MODE_128K) && defined (COMPRESSED_LEVELS)
			if (py == 0 && pvy < 0 && n_pant >= level_data->map_w) {
				n_pant -= level_data->map_w;
#else
			if (py == 0 && pvy < 0 && n_pant >= MAP_W) {
				n_pant -= MAP_W;
#endif
				draw_scr ();
				py = 9216;	
			}
			if (py == 9216 && pvy > 0) {				// 9216 = 144 * 64
#if defined (MODE_128K) && defined (COMPRESSED_LEVELS)
